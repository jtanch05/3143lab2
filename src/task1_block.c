...
// Each MPI process checks a contiguous block of the 6k+-1 candidates.
...
#define OUTPUT_FILE "task1_block_primelist.txt"
...
int main(int argc, char *argv[])
{
    ...
    // Each rank checks one contiguous range of candidate pairs.
...
// Counts the contiguous block of candidate pairs assigned to this process.
long long count_local_pairs(int n, int my_rank, int p)
{
    // Pairs start at 5, 11, 17, ... and must start strictly below n.
    long long total_pairs = (n > 5) ? ((long long)n - 6) / 6 + 1 : 0;
    long long start_pair = total_pairs * my_rank / p;
    long long end_pair = total_pairs * (my_rank + 1) / p;
    long long local_pairs = end_pair - start_pair;

    return local_pairs;
}


// Searches the contiguous block of candidate pairs assigned to this process.
int find_local_primes(int n, int my_rank, int p,
                      int *small_primes, int small_count,
                      int *local_primes)
{
    ...
    for(long long pair = start_pair; pair < end_pair; pair++)
    {
        int k = (int)(5 + 6 * pair);
